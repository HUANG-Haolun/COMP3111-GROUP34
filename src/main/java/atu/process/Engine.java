...
public class Engine {
	 ...
	 * @param input The list of Person instances that is holding all the data about
	 *              the students.
	 */
	...
	/**
	 * The method that returns the list attribute of the Engine instance.
	 * 
	...
	/**
	 * The method that returns the K1_list attribute of the Engine instance.
	 * 
	...
	/**
	 * The method that returns the K2_list attribute of the Engine instance.
	 * 
	 * @return List instance of {@link Engine#K1_list}
	 */
	...
	/**
	 * The method that returns the K3_list attribute of the Engine instance.
	 * 
	 * @return List instance of {@link Engine#K1_list}
	 */
	 ...
	 * K1_energy, then Person instance with lower K2_energy will be in previous
	 * order.
	 ...
	 * K2_energy, then Person instance with lower K1_energy will be in previous
	 * order.
	 ...
	 * K2_energy, then Person instance with lower K1_energy will be in previous
	 * order.
	 * Finally, the first one-third Person instances of the sorted list which are
	 * not
	...
	/**
	 * The method that first copies value of the parameter and uses it to sort a
	 * list
	 * into the Person instances with descending order. Then, the remaining number
	 * of
	 * the students after division with 3 from the size of the list is calculated
	 * and
	 ...
	 * 
	 * After that, since the original list must be obtained, the list of Engine
	 * class
	 ...
	 * team and the leader is chosen by comparing the preferences of being the
	 * leader
			...
			if (a.getMypreference() == 1) {
				if (b.getMypreference() == 1) {
					if (c.getMypreference() == 1)
						...
						leader = (a_K2 >= b_K2 ? a : b).getStudentname();
				} else {
					if (c.getMypreference() == 1)
						leader = (a_K2 >= c_K2 ? a : c).getStudentname();
					else
						leader = a.getStudentname();
				}
			} else {
				if (b.getMypreference() == 1) {
					if (c.getMypreference() == 1)
	...
	/**
	 * The method that returns the Team instance by using the team id as the
	 * parameter.
	 * 
